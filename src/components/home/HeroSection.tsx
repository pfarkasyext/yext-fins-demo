...
const HeroSection = ({ img }: HeroSectionProps) => {
  return (
    <div className="relative isolate over-flow-hidden w-full h-[500px]">
      <Image
        image={img}
        className="absolute inset-0 -z-10 h-full min-w-full object-cover"
      />
      ...
      <div className="w-full h-full flex flex-col justify-center items-end px-20">
        <div className="p-4 rounded-md shadow bg-white w-96 flex flex-col gap-8">
          ...
          <Button variant="default">Find A Wealth Advisor</Button>
        </div>
      </div>
    </div>
  );
